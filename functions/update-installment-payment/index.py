            ...
            if installment_wallet_id is not None and str(installment_wallet_id).strip() != '':
                ...
                if paid_delta != Decimal('0'):
                    ...
                    if paid_minor_units != 0 and current_balance is not None:
                        ...
                        spent_rs = tx.execute(spent_q, {'$wallet_id': installment_wallet_id, '$user_id': installment_user_id})
                        from decimal import Decimal as _D
                        spent_sum_dec = _D(str(spent_rs[0].rows[0].spent_sum or 0)) if spent_rs[0].rows else _D('0')
                        spent_mu = int((spent_sum_dec * _D('100')).quantize(_D('1'), rounding=ROUND_HALF_UP))
