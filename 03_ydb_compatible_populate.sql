-- =====================================================
-- YDB COMPATIBLE VERSION - USING PROPER YDB SYNTAX
-- RUN EACH STATEMENT ONE BY ONE
-- =====================================================

-- 1. Update paid_amount using YDB UPSERT syntax
$paid_amounts = (
    SELECT 
        p.installment_id AS id,
        SUM(p.expected_amount) AS paid_amount
...
SELECT id, COALESCE(paid_amount, 0.0) AS paid_amount FROM $paid_amounts;

-- 2. Fix NULL paid_amount
UPSERT INTO installments (id, paid_amount)
SELECT id, 0.0 AS paid_amount 
FROM installments 
...
SELECT 
    id, 
    installment_price - COALESCE(paid_amount, 0.0) AS remaining_amount
FROM installments;
